...
- **`CubeCalibrationGuardTest`** (non-gated, fast): over the fixed-seed `selfPlayTrajectory` helper + seed set already used by `GammonCalibrationGuardTest` (n4 — deterministic), the computed take point stays within [0.18, 0.27] for both phase `x` values, and `TOO_GOOD` only fires when `cubelessEquity > 1.0`.
